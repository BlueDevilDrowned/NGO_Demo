...
#region Configuration
        ...
        private PlayableGraph GetSelectedGraphInToolBar(List<PlayableGraph> graphs, PlayableGraph currentGraph)
        {
            if (graphs == null || graphs.Count == 0)
                return new PlayableGraph(); // Unity 6 has no PlayableGraph.Null constant.
            ...
            int newSelection = EditorGUILayout.Popup(Mathf.Clamp(currentSelection, 0, options.Count - 1), options.ToArray(), GUILayout.Width(200));
        ...
        {
            if (m_Graphs != null && graph.IsValid() && !m_Graphs.Contains(graph))
                m_Graphs.Add(graph);
        }
        ...
        private List<PlayableGraph> GetGraphList()
        {
            ...
            if (selectedGraphs.Count == 0 && m_Graphs != null)
                selectedGraphs = m_Graphs.Where(graph => graph.IsValid()).ToList();
