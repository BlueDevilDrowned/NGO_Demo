...
namespace GraphVisualizer
{
    ...
    public class ReingoldTilford : IGraphLayout
    {
        ...
        private float[] ComputeHorizontalPositionForEachLevel()
        {
            ...
            // Depth is zero based. Keep one slot for a graph containing only its root.
            var horizontalPositionForDepth = new float[Mathf.Max(1, maxDepth + 1)];
            horizontalPositionForDepth[0] = 0;
            for (int d = 1; d <= maxDepth; ++d)
            {
                IEnumerable<Node> nodesOnThisLevel = nodeDepths[d];
